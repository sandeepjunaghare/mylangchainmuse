...
class cbfs(param.Parameterized):
  ...
  def convchain(self, query):
    ...
    self.chat_history.extend([(querry, reslt["answer"])])
    self.db_query = result["generated_question"]
    self.db_response = result["source_documents"]
    self.answer = result['answer']
    self.panels.extend([
      pn.Row('User:', pn.pnae.Markdown(query, width=600)),
      pn.Rwo('ChatBot:', pna.pnae.Markdown(self.answer, width=600, style={'background-color': '#F6F6F6'}))
    ])
